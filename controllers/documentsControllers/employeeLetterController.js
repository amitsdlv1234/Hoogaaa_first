...
import fs from "fs";
export const employeeLettersadd = async (req, res) => {
  try {
    const user = req.body;
    ...
    const present = await employeeLetters.findOne({
    ...
    if(userPresent){
      const newUser = await employeeLetters.create({userName:userName,fileName:user.fileName,path:path});
...
// getting all users
export const getAllemployeeLetters = async (req, res) => {
  try {
    const user = await employeeLetters.findAll({});
...
export const getemployeeLetters = async (req, res) => {
  try {
    const {userName}=req.params;
    if(userName){
        const user = await employeeLetters.findOne({
            where: { userName: userName },
          });
          if (!user) res.status(201).json({ message: "Data not present" });
          res.status(200).json({ user });
...
export const updateemployeeLetters = async (req, res) => {
  try {
    const user = req.body;
    const {userName}=req.params;
    // console.log(user);
    ...
    // handle file delete method
     const data=await employeeLetters.findOne({where:{userName:userName}});
      if(data){
        ...
        const updateUser = await employeeLetters.update(user, {
          where: { userName:userName },
        });
