...
import fs from "fs";
export const offerLettersadd = async (req, res) => {
  try {
    const user = req.body;
    ...
    const present = await offerLetters.findOne({
    ...
    if(userPresent){
      const newUser = await offerLetters.create({userName:userName,fileName:user.fileName,path:path});
...
export const getAllofferLetters = async (req, res) => {
  try {
    const user = await offerLetters.findAll({});
...
export const getofferLetters = async (req, res) => {
  try {
    const {userName}=req.params;
    if(userName){
        const user = await offerLetters.findOne({
            where: { userName: userName },
          });
          if (!user) res.status(201).json({ message: "Data not present" });
          res.status(200).json({ user });
...
export const updateofferLetters = async (req, res) => {
  try {
    const user = req.body;
    const {userName}=req.params;
    // console.log(user);
    ...
    // handle file delete method
     const data=await offerLetters.findOne({where:{userName:userName}});
      if(data){
        ...
        const updateUser = await offerLetters.update(user, {
          where: { userName:userName },
        });
