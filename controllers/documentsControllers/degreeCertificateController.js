...
import fs from "fs";
export const degreeCertificatesadd = async (req, res) => {
  try {
    const user = req.body;
    ...
    const present = await degreeCertificates.findOne({
    ...
    if(userPresent){
      const newUser = await degreeCertificates.create({userName:userName,fileName:user.fileName,path:path});
...
// getting all users
export const getAlldegreeCertificates = async (req, res) => {
  try {
    const user = await degreeCertificates.findAll({});
...
export const getdegreeCertificates = async (req, res) => {
  try {
    const {userName}=req.params;
    if(userName){
        const user = await degreeCertificates.findOne({
            where: { userName: userName },
          });
          if (!user) res.status(201).json({ message: "Data not present" });
          res.status(200).json({ user });
...
export const updatedegreeCertificates = async (req, res) => {
  try {
    const user = req.body;
    const {userName}=req.params;
    // console.log(user);
    ...
    // handle file delete method
     const data=await degreeCertificates.findOne({where:{userName:userName}});
      if(data){
        ...
        const updateUser = await degreeCertificates.update(user, {
          where: { userName:userName },
        });
